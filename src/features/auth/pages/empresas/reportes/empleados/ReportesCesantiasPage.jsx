...
export default function ReportesCesantiasPage() {
  ...
  const { datos: datosCesantias, cargando: cargandoCes } =
    useHistoricos(historicosService.getReporteCesantiasEmpleados, paramsBase);

  const { datos: datosPeriodo, cargando: cargandoPer } =
    useHistoricos(historicosService.getReporteCesantiasConsolidado, paramsBase);
  ...
  const rawDatos     = tabPrincipal === 'cesantias' ? datosCesantias : datosPeriodo;
